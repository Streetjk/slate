...
Current stage: 8D1M-G G2 exact-image preflight
Current status: G2_BLOCKED_EXACT_IMAGE_CANNOT_BE_LOADED_SAFELY — the exact ARM64 candidate is local but absent from the remote Docker store; zero-provider transfer hit the remote Docker root capacity boundary and was cleaned up without production impact.
G0_PROVIDER_SESSIONS_USED=2_OF_3
G0_NEXT_ACTION=HUMAN_INFRASTRUCTURE_DECISION_FOR_EXACT_IMAGE_AVAILABILITY
...
G1_READY_FOR_G2=YES_BUT_EXACT_IMAGE_REMOTE_AVAILABILITY_BLOCKS_PREFLIGHT
