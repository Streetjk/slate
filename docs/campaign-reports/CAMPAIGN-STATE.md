...
Current stage: 8D1M-G G2 NVMe-isolated daemon replay
Current status: G2_BLOCKED_ISOLATED_RUNC_BOOTSTRAP — the exact ARM64 image loaded into an isolated NVMe-backed Docker daemon, but both it and a trivial base-image control fail before process start at the nested runc bootstrap boundary; no provider call occurred.
...
G1_READY_FOR_G2=YES_BUT_ISOLATED_RUNC_BOOTSTRAP_BLOCKS_PROCESS_START
...
G2_ISOLATED_DAEMON=PASS_DOCKER_29_1_3_OVERLAY2_NVME_ROOT
...
G2_CREDENTIAL_VALUE_READ=NO
G2_PRODUCTION_MUTATION=NO
G2_DISPOSABLE_CLEANUP=PASS
G3_STATUS=NOT_RUN_G2_RUNTIME_FAILED
READY_FOR_G2_PROVIDER_CALL=NO_RUNTIME_BOUNDARY
