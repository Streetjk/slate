...
Current stage: 8D1M-G G2 host-native NVMe provider validation
Current status: G2_FAILED_CONNECT_TIMEOUT — the exact ARM64 image launched successfully in a host-native disposable NVMe-backed Docker daemon, the provider-disabled candidate matrix passed, and the one authorized Gemini session produced only a sanitized CONNECT_TIMEOUT. G3 was not run.
...
G_CONTINUATION_DIRECTIVE_STATE=AUTHORIZED_ACTIVE_STOPPED_AFTER_G2_PROVIDER_FAILURE
G2_STATUS=FAIL_PROVIDER_CONNECT_TIMEOUT
...
G2_ISOLATED_DAEMON=PASS_DOCKER_29_1_3_OVERLAY2_NVME_ROOT_HOST_NATIVE
