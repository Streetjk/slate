...
# Campaign 8D1M-G M2. Migrate only containerd's persistent root. Preserve
# /var/lib/containerd, /var/lib/docker, the active NVMe Docker root, and
# all Deluge paths/data. Do not load the UX image or recreate Slate here.
...
DROPIN=$DROPIN_DIR/99-slate-m2-nvme-root.conf
...
log() {
  printf 'M2_ROOT_STEP_V1 stage=%s status=%s\n' "$1" "$2"
}
...
rollback() {
  ...
  if [[ "$(systemctl is-active docker 2>/dev/null || true)" == active ||
    ...
    printf 'M2_ROOT_STEP_V1 rollback=FAIL action=STOP_SERVICES\n'
    failure_class=$saved_failure
    return 1
  fi

  if (( dropin_created )); then
    rm -f -- "$DROPIN" "$DROPIN_TMP" >/dev/null 2>&1 || true
    if test -e "$DROPIN" || test -e "$DROPIN_TMP"; then
      printf 'M2_ROOT_STEP_V1 rollback=FAIL action=RESTORE_SERVICE_CONFIGURATION\n'
      failure_class=$saved_failure
      return 1
    fi
    systemctl daemon-reload >/dev/null 2>&1 || {
      printf 'M2_ROOT_STEP_V1 rollback=FAIL action=DAEMON_RELOAD\n'
      failure_class=$saved_failure
      return 1
    }
  fi
  if (( dropin_dir_created )); then
    rmdir "$DROPIN_DIR" >/dev/null 2>&1 || true
    if test -e "$DROPIN_DIR"; then
      printf 'M2_ROOT_STEP_V1 rollback=FAIL action=RESTORE_DROPIN_DIRECTORY\n'
      failure_class=$saved_failure
      return 1
    fi
  fi
  ...
  systemctl start containerd >/dev/null 2>&1 || {
    printf 'M2_ROOT_STEP_V1 rollback=FAIL action=START_CONTAINERD\n'
    failure_class=$saved_failure
    return 1
  }
  ...
  while [[ "$(systemctl is-active containerd 2>/dev/null || true)" != active ]]; do
    if (( SECONDS >= deadline )); then
      printf 'M2_ROOT_STEP_V1 rollback=FAIL action=CONTAINERD_NOT_ACTIVE\n'
      failure_class=$saved_failure
      return 1
    fi
    sleep 1
  ...
  systemctl start docker >/dev/null 2>&1 || {
    printf 'M2_ROOT_STEP_V1 rollback=FAIL action=START_DOCKER\n'
    failure_class=$saved_failure
    return 1
  }
  if wait_for_health "$DOCKER_ROOT" "$slate_restart_before" "$mysql_restart_before" "$ROLLBACK_TIMEOUT"; then
    printf 'M2_ROOT_STEP_V1 rollback=PASS containerd_root=%s docker_root=%s health=PASS\n' "$SOURCE" "$DOCKER_ROOT"
    failure_class=$saved_failure
    return 0
  fi
  printf 'M2_ROOT_STEP_V1 rollback=FAIL containerd_root=%s docker_root=%s health=FAIL\n' "$SOURCE" "$DOCKER_ROOT"
  failure_class=$saved_failure
  return 1
}
...
handle_failure() {
  ...
  printf 'M2_ROOT_STEP_V1 stage=%s status=FAIL class=%s\n' "$stage" "$failure_class"
  exit "$exit_code"
}
...
for command_name in docker containerd rsync findmnt realpath curl systemctl du df stat sha256sum find mktemp sort wc awk grep sleep mkdir cp chmod chown rm mv rmdir diff cat; do
...
test ! -e "$DROPIN" || { failure_class=DROPIN_COLLISION; false; }
...
manifest_metrics "$SOURCE" SRC >"$tmpdir/src.metrics"
manifest_metrics "$DEST" DST >"$tmpdir/dst.metrics"
...
stage=switch
if test -e "$DROPIN_DIR"; then
  test -d "$DROPIN_DIR" || { failure_class=DROPIN_DIR_NOT_DIRECTORY; false; }
else
  mkdir "$DROPIN_DIR" || { failure_class=DROPIN_DIR_CREATE_FAILED; false; }
  dropin_dir_created=1
fi
test ! -L "$DROPIN_DIR" || { failure_class=DROPIN_DIR_SYMLINK; false; }
cat >"$DROPIN_TMP" <<EOF
[Service]
ExecStart=
ExecStart=/usr/bin/containerd --root $DEST --state $STATE
EOF
chmod 644 "$DROPIN_TMP" || { failure_class=DROPIN_TMP_CHMOD_FAILED; false; }
chown root:root "$DROPIN_TMP" || { failure_class=DROPIN_TMP_CHOWN_FAILED; false; }
mv -- "$DROPIN_TMP" "$DROPIN" || { failure_class=DROPIN_INSTALL_FAILED; false; }
dropin_created=1
...
wait_for_health "$DOCKER_ROOT" "$slate_restart_before" "$mysql_restart_before" "$STARTUP_TIMEOUT" || false
...
trap - ERR INT TERM HUP
printf 'M2_ROOT_STEP_V1 stage=switch status=PASS containerd_root=%s state=%s docker_root=%s old_root_preserved=YES health=PASS\n' "$DEST" "$STATE" "$DOCKER_ROOT"
printf 'M2_ROOT_STEP_V1 stage=complete status=PASS\n'
