...
# Campaign 8D1M-G M1 recovery.  This is a versioned V3 replacement for
# /home/pi/slate-m1-rootstep.sh and V2.  It never removes either Docker tree.
SOURCE=/var/lib/docker
DEST=/mnt/ssd-tmp/slate-tools/docker-data
NVME=/mnt/ssd-tmp
SLATE_TOOLS=/mnt/ssd-tmp/slate-tools
DAEMON_CONFIG=/etc/docker/daemon.json
DAEMON_BACKUP=/mnt/ssd-tmp/slate-tools/m1-daemon-pre-migration-v3.json
VERIFY_FILE=/mnt/ssd-tmp/slate-tools/m1-root-copy-v3-verify.txt
...
log() {
  printf 'M1_ROOT_STEP_V3 stage=%s status=%s\n' "$1" "$2"
}
...
rollback() {
  ...
  if (( rollback_ok )); then
    printf 'M1_ROOT_STEP_V3 rollback=PASS data_root=%s health=PASS\n' "$SOURCE"
  else
    printf 'M1_ROOT_STEP_V3 rollback=FAIL data_root=%s health=FAIL\n' "$root_after"
  fi
...
on_error() {
  ...
  printf 'M1_ROOT_STEP_V3 stage=%s status=FAIL class=%s\n' "$stage" "$failure_class"
  exit "$rc"
}
trap on_error ERR
...
tmpdir=$(mktemp -d "$SLATE_TOOLS/m1-root-copy-v3-metrics.XXXXXX")
...
trap - ERR
printf 'M1_ROOT_STEP_V3 stage=switch status=PASS data_root=%s old_root_preserved=YES\n' "$DEST"
printf 'M1_ROOT_STEP_V3 stage=complete status=PASS\n'
