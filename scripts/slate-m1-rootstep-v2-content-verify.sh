...
# Campaign 8D1M-G M1 recovery.  This is a versioned replacement for
# /home/pi/slate-m1-rootstep.sh.  It never removes either Docker tree.
SOURCE=/var/lib/docker
DEST=/mnt/ssd-tmp/slate-tools/docker-data
NVME=/mnt/ssd-tmp
SLATE_TOOLS=/mnt/ssd-tmp/slate-tools
DAEMON_CONFIG=/etc/docker/daemon.json
DAEMON_BACKUP=/mnt/ssd-tmp/slate-tools/m1-daemon-pre-migration-v2.json
VERIFY_FILE=/mnt/ssd-tmp/slate-tools/m1-root-copy-v2-verify.txt
...
log() {
  printf 'M1_ROOT_STEP_V2 stage=%s status=%s\n' "$1" "$2"
}
...
rollback() {
  local rollback_ok=1
  set +e
  systemctl stop docker >/dev/null 2>&1 || rollback_ok=0
  if (( config_was_present )); then
    cp -a "$DAEMON_BACKUP" "$DAEMON_CONFIG" >/dev/null 2>&1 || rollback_ok=0
    chmod "$config_mode" "$DAEMON_CONFIG" >/dev/null 2>&1 || rollback_ok=0
    chown root:root "$DAEMON_CONFIG" >/dev/null 2>&1 || rollback_ok=0
  else
    rm -f -- "$DAEMON_CONFIG" >/dev/null 2>&1 || rollback_ok=0
  fi
  systemctl start docker >/dev/null 2>&1 || rollback_ok=0
  local root_after
  root_after=$(docker info --format '{{.DockerRootDir}}' 2>/dev/null) || rollback_ok=0
  [[ "$root_after" == "$SOURCE" ]] || rollback_ok=0
  if (( rollback_ok )); then
    printf 'M1_ROOT_STEP_V2 rollback=PASS data_root=%s\n' "$SOURCE"
  else
    printf 'M1_ROOT_STEP_V2 rollback=FAIL\n'
  fi
...
on_error() {
  ...
  printf 'M1_ROOT_STEP_V2 stage=%s status=FAIL class=%s\n' "$stage" "$failure_class"
  exit "$rc"
}
trap on_error ERR
...
assert_production_health() {
  ...
  [[ "$(docker inspect slate-note4 --format '{{.RestartCount}}')" == 0 ]] || { failure_class=SLATE_RESTART_COUNT_NONZERO; return 1; }
  [[ "$(docker inspect slate-note4-mysql --format '{{.RestartCount}}')" == 0 ]] || { failure_class=MYSQL_RESTART_COUNT_NONZERO; return 1; }
...
tmpdir=$(mktemp -d "$SLATE_TOOLS/m1-root-copy-v2-metrics.XXXXXX")
...
for _ in $(seq 1 60); do
  [[ "$(docker info --format '{{.DockerRootDir}}' 2>/dev/null || true)" == "$DEST" ]] && break
  sleep 1
done
...
trap - ERR
printf 'M1_ROOT_STEP_V2 stage=switch status=PASS data_root=%s old_root_preserved=YES\n' "$DEST"
printf 'M1_ROOT_STEP_V2 stage=complete status=PASS\n'
