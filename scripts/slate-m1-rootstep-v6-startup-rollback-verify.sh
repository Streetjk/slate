...
# Campaign 8D1M-G M1 recovery.  This is a versioned V6 replacement for
# /home/pi/slate-m1-rootstep.sh, V2, V3, V4, and V5.  It never removes either Docker tree.
SOURCE=/var/lib/docker
DEST=/mnt/ssd-tmp/slate-tools/docker-data
NVME=/mnt/ssd-tmp
SLATE_TOOLS=/mnt/ssd-tmp/slate-tools
DAEMON_CONFIG=/etc/docker/daemon.json
DAEMON_BACKUP=/mnt/ssd-tmp/slate-tools/m1-daemon-pre-migration-v6.json
VERIFY_FILE=/mnt/ssd-tmp/slate-tools/m1-root-copy-v6-verify.txt
...
log() {
  printf 'M1_ROOT_STEP_V6 stage=%s status=%s\n' "$1" "$2"
}
...
rollback() {
  ...
  if [[ "$(systemctl is-active docker 2>/dev/null || true)" == active ]]; then
    printf 'M1_ROOT_STEP_V6 rollback=FAIL action=STOP_DOCKER\n'
    return 1
  fi
  ...
  if test -e "$DAEMON_CONFIG.tmp"; then
    ...
    if test -e "$DAEMON_CONFIG.tmp"; then
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=REMOVE_TMP_CONFIG\n'
      return 1
    fi
  fi
  ...
  if (( config_was_present )); then
    if ! test -f "$DAEMON_BACKUP"; then
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=BACKUP_CONFIG_MISSING\n'
      return 1
    fi
    cp -a "$DAEMON_BACKUP" "$DAEMON_CONFIG" >/dev/null 2>&1 || {
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=RESTORE_CONFIG\n'
      return 1
    }
    chmod "$config_mode" "$DAEMON_CONFIG" >/dev/null 2>&1 || {
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=CHMOD_CONFIG\n'
      return 1
    }
    chown root:root "$DAEMON_CONFIG" >/dev/null 2>&1 || {
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=CHOWN_CONFIG\n'
      return 1
    }
    if ! test -f "$DAEMON_CONFIG"; then
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=VERIFY_RESTORED_CONFIG\n'
      return 1
    fi
    if [[ "$(sha256sum "$DAEMON_BACKUP" | awk '{print $1}')" != "$(sha256sum "$DAEMON_CONFIG" | awk '{print $1}')" ]]; then
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=VERIFY_CONFIG_CONTENT\n'
      return 1
    fi
  else
    rm -f -- "$DAEMON_CONFIG" >/dev/null 2>&1 || true
    if test -e "$DAEMON_CONFIG"; then
      printf 'M1_ROOT_STEP_V6 rollback=FAIL action=REMOVE_CONFIG\n'
      return 1
    fi
  fi
  ...
  systemctl start docker >/dev/null 2>&1 || {
    printf 'M1_ROOT_STEP_V6 rollback=FAIL action=START_DOCKER\n'
    return 1
  }
  if [[ "$(systemctl is-active docker 2>/dev/null || true)" != active ]]; then
    printf 'M1_ROOT_STEP_V6 rollback=FAIL action=START_DOCKER\n'
  ...
  if (( rollback_ok )); then
    printf 'M1_ROOT_STEP_V6 rollback=PASS data_root=%s health=PASS\n' "$SOURCE"
  else
    printf 'M1_ROOT_STEP_V6 rollback=FAIL data_root=%s health=FAIL\n' "$root_after"
  fi
...
on_error() {
  ...
  printf 'M1_ROOT_STEP_V6 stage=%s status=FAIL class=%s\n' "$stage" "$failure_class"
  exit "$rc"
}
trap on_error ERR
...
tmpdir=$(mktemp -d "$SLATE_TOOLS/m1-root-copy-v6-metrics.XXXXXX")
...
trap - ERR
printf 'M1_ROOT_STEP_V6 stage=switch status=PASS data_root=%s old_root_preserved=YES\n' "$DEST"
printf 'M1_ROOT_STEP_V6 stage=complete status=PASS\n'
