...
<div class="wrap">
  <header class="masthead">
    <div class="kicker">
      <span>VOL. 0 · SETUP PORTAL</span>
      <span>{{AP_SSID}}</span>
    </div>
    <h1 class="brand">Slate<span class="brand-dot">.</span></h1>
    <p class="tag">Your desk e-paper display needs its credentials first.</p>
    <div class="rule"></div>
  ...
  <form id="f">
    <section>
      ...
      <ul id="nets" class="networks">
        <li class="placeholder"><span class="ssid">Scanning</span><span class="meta"><span class="spin"></span></span></li>
      </ul>
      <label>
        <span class="lbl">ssid</span>
        <input id="ssid" name="ssid" type="text" required maxlength="32" placeholder="Select a network above or enter it manually">
      </label>
      <label>
        <span class="lbl">password</span>
        <input id="password" name="password" type="password" maxlength="64" placeholder="Leave empty for an open network">
      </label>
    </section>
    ...
    <section>
      ...
      <p class="hint">Enter the address of the Slate backend. For local testing, use http://&lt;LAN-IP&gt;:3001.</p>
    </section>
...
async function scan() {
  ...
  try {
    ...
    ul.innerHTML = '';
    if (!list.length) {
      ul.innerHTML = '<li class="placeholder"><span class="ssid">No networks found</span><span class="meta">—</span></li>';
      return;
    }
    ...
    $('nets').innerHTML = '<li class="placeholder"><span class="ssid">Scan failed · retrying</span><span class="meta"><span class="spin"></span></span></li>';
  } finally {
    scanning = false;
  }
}
...
function showSuccess(ssid) {
  ...
  eyebrow.textContent = '✓ SETUP COMPLETE · PAIRED';
  const h3 = document.createElement('h3');
  h3.textContent = 'The device will restart shortly.';
  const p1 = document.createElement('p');
  p1.textContent = 'The device will connect to ';
  const strong = document.createElement('strong');
  strong.textContent = ssid;
  p1.appendChild(strong);
  p1.appendChild(document.createTextNode(' and restart in a few seconds.'));
  const p2 = document.createElement('p');
  p2.textContent = 'The device screen will also show setup completion and the restart countdown.';
  const p3 = document.createElement('p');
  p3.className = 'cd';
  p3.textContent = 'This page will disconnect in ';
  const cdSpan = document.createElement('span');
  cdSpan.id = 'cd';
  cdSpan.textContent = '5';
  p3.appendChild(cdSpan);
  p3.appendChild(document.createTextNode(' seconds; you can close it.'));
...
$('f').addEventListener('submit', async (e) => {
  ...
  if (!body.ssid) { setStatus('SSID cannot be empty', 'err'); return; }
  if (!body.server_url) { setStatus('Server URL cannot be empty', 'err'); return; }
  $('btn').disabled = true;
  setStatus('Trying ' + body.ssid + ' … (up to 10 seconds)', 'info');
  try {
    ...
    if (!r.ok || !data.success) {
      setStatus((data.error || ('HTTP ' + r.status)) + ' · Check the password and try again', 'err');
      $('btn').disabled = false;
      return;
    }
    ...
    setStatus('Network error: ' + (err && err.message || 'unknown') + ' · Try again', 'err');
    $('btn').disabled = false;
  }
});
</script>
