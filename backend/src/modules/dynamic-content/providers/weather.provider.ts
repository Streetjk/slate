...
export class WeatherProvider implements DataProvider<WeatherConfigT, WeatherProviderData> {
  ...
  validateConfig(raw: unknown): WeatherConfigT {
    const result = WeatherConfig.safeParse(raw);
    if (!result.success) {
      const first = result.error.issues[0];
      const path = first?.path.length ? first.path.join('.') : 'config';
      throw new Error(`Invalid ${path}: ${first?.message ?? 'malformed weather configuration'}`);
    }
    return result.data;
  }
