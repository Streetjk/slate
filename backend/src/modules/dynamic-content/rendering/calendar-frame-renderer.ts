...
import { getPath, isRecord, pad2, pickText } from './helpers/frame-value-utils';
...
export function renderDailyCalendarFrame(
  ...
  const weekday = pickText(data.weekday, englishWeekday(ctx.renderedAt, tz));
  const publicHoliday = pickText(data.publicHoliday, '');
  const monthDay = `${pad2(Number(day))} ${monthName(Number(month))}`;
  ...
  const rightY = upperTop + Math.round((upperBottom - upperTop - fonts.sans16.lineHeight) / 2);
  const dateX = 18;
  const rightX = 202;
  ...
  draw.drawText(c, fonts.sans16, weekday || 'Unknown day', rightX, rightY, {
    maxWidth: CONTENT_RIGHT - rightX,
    ellipsis: true,
  });
  if (publicHoliday) {
    draw.drawText(c, fonts.sans16, publicHoliday, rightX, rightY + 30, {
      maxWidth: CONTENT_RIGHT - rightX,
      ellipsis: true,
    });
  }
  ...
  drawInfoRow(
    c,
    fonts,
    publicHoliday ? 'Public holiday' : 'Perth calendar',
    publicHoliday || 'Australia/Perth · en-AU',
    CONTENT_LEFT,
    ruleY + 20,
    CONTENT_WIDTH,
    draw
...
export function renderMonthCalendarFrame(
  ...
  // The status-bar caption already carries the active month/year. Avoid
  // repeating it inside the bitmap and spend that vertical space on the grid.
  const x0 = 14;
  const y0 = CONTENT_SAFE_TOP + 8;
  const w = FRAME_WIDTH - 28;
  const colW = Math.floor(w / 7);
  const headerH = 24;
  const weekRows = Math.ceil((first + total) / 7);
  const rowBottomReserve = 30;
  const rowH =
    weekRows > 1
      ? Math.floor((FRAME_HEIGHT - (y0 + headerH) - rowBottomReserve) / (weekRows - 1))
      : 42;
...
function englishWeekday(date: Date, timeZone: string): string {
  return new Intl.DateTimeFormat('en-AU', { weekday: 'long', timeZone }).format(date);
}
